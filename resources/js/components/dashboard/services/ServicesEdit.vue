											...
											<v-tab-item >
												<v-card flat>
													<v-card-text>
														<v-row >
															<v-col cols="12"  >
																<v-text-field v-model="editedItem.title_en" label="Title En*"  filled  ></v-text-field>
															</v-col>
															<v-col  cols="12">
											...
											<v-tab-item >
												<v-card flat >
													<v-card-text>
														<v-row >
															<v-col cols="12"  >
																<v-text-field v-model="editedItem.title_ar" label="Title Ar*"  filled  ></v-text-field>
															</v-col>
															<v-col  cols="12">
			...
			if(this.$route.params.id)
			{
				...
				{   
					...
					let { data } = await axios({
						method: "get",
						url: "/app/services/"+this.$route.params.id
					});
                    this.editedItem = Object.assign({}, data);
                    this.editedItem.banner=[]
                    for(let p of data.banner)
					{
						this.editedItem.banner.push(p.image)
					}
		...
		async save() 
		{
			this.loading = true;
				...
				try 
				{
					let { data } = await axios({
						method: "put",
						url: "/app/services/"+this.editedItem.id,
